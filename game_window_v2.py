...
class GameWindow:
    ...
    def main(self):
        ...
        while running:
                    ...
                    if event.key == pygame.K_SPACE:  # Draw card on SPACE key press
                        if self.player_guess:
                            ...
                            if self.drawn_card:
                                ...
                                if self.player_guess.lower() == card_name.lower():
                                    self.is_guess_correct = True
                                    self.new_points_calc *= 1.5
                                else:
                                    self.is_guess_correct = False
                                    self.new_points_calc *= 0.85
                                self.player_guess = ""  # Reset guess after drawing
            ...
            guess_text = f'GUESS: {self.player_guess}'
            self.draw_text(guess_text, self.screen_width // 5, self.screen_height - 100, self.BLACK)

            # Display whether the guess was correct
            if self.drawn_card:
                result_text = 'Correct!' if self.is_guess_correct else 'Incorrect!'
